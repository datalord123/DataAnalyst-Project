...
### Description Two
The plot on the top is a histogram that shows the total number of contributions
received by each candidate. The plot below it is a density plot that effectively
showsthe distribution of contributions for each candidate.

So for instance, we can see that while 
Jeb Bush is the weakest of the 'frontrunners'in terms of the number of 
his donations, the contributions he does have are quite large. 
This differentiates him from the remaining candidates as the 
distribution of their contributions are centered further to the left, and are
spread more evenly(less of a spike) across both low and high values.
...
```
# Description Three
The plot above is a histogram that show the total of contributions made for 
each candidate broken down by bucket. The 'buckets' in this instance, are
