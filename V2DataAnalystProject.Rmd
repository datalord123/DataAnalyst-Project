---
title: "Analysis of 2016 NC Presidential Campaign Contributions by Matthew Bellissimo"
output:
  html_document:
    theme: readable
---
```
References: http://www.realclearpolitics.com/epolls/2016/president/us/2016_republican_#presidential_nomination-3823.html,http://www.realclearpolitics.com/epolls/2016/preside#nt/us/2016_democratic_presidential_nomination-3824.html
```
```{r echo=FALSE, message=FALSE, warning=FALSE,packages}
#Could also add fig.align = 'center' to R block header
#Load the Packages
library(ggplot2)
library(gridExtra)
library(dplyr)
...
```{r echo=FALSE, message=FALSE, warning=FALSE}
#Contribtion amount by Frequency
...
##Good place to add correlations between candidates
```
...
This is a large number for a legend, and i can only image that the color
will be effectively uninterpretable. 

To mitigate this issue I decided to go back to using the Contributor dataset
that excluded outliers. I also chose to scale the the y axis to log10
and used CumSum  to calculate the y value as the amount collected to date,
instead of trying to track all the hundreds of individual contribution data 
points.
...
ContributionsNO<-ContributionsNO %>% group_by(cand_nm)%>%
  ...
  group_by(cand_nm,Months)%>%
  mutate(MonthlyTotal=sum(contb_receipt_amt))
...
After looking at this plot, I also started to wonder how and whether
the Candidates could be correlated with each other.
```{r echo=FALSE, message=FALSE, warning=FALSE}
#Create DataFrame for correlations.
CorTable<-ContributionsNO%>%group_by(cand_nm,Days)%>%
  summarise(TotalRaise=mean(TotalRaised))%>%
  arrange(Days)
#Spread dataframe so that each cand_nm level becomes a variable
CorTable<-spread(CorTable,cand_nm,TotalRaise)
#Exclude rows that include days prior to 286(The day the Bush, the
# last candidate) entered the race.
CorTable<-subset(CorTable,Days>=286)
CorTable$Days<- NULL
#Because we're dealing with dollar values and because candidates
#started at different points in the dates. It probably makes sense 
# to transform the data with log().
CorLogTable<-log(CorTable)
cormat<-round(cor(CorLogTable,use="complete"),2)
#Now we just have to get rid of the redundant information
#I'll do this by creating a function to
# grab the lower triangle of the correlation matrix
get_lower_tri<-function(cormat){
  cormat[upper.tri(cormat)]<-NA
  return(cormat)
}

#Now let's create a function to reorder the correlation matrix by value.
...
#Finally we can visualize the matrix with geom_tile
ggplot(data = melted_cormat,aes(x=Var1,y=Var2,fill=value)) +
  geom_tile()+
  geom_text(aes(Var1,Var2,label = value),color = 'white',size = 4)+ 
  ggtitle('Correlation Matrix')+
  theme(axis.text.x=element_text(angle=20, size=10, vjust=0.9,hjust=.9),
...
In the plot above, we can see that all the candidates are very strongly
correlted with each other. Even the 'less correlated' candidate pairs
[Ted Cruz,Rand Paul] and [Jeb Bush,Rand Paul] still have a positive correlation
of over .6 percent, which is High.

Now I wanted to look and Mean an Median Contributions per Month.
```{r echo=FALSE, message=FALSE, warning=FALSE}
...
### Description One
The lines on the plot are the accummulated total contributions of each
candidate. The dot's on the plot are the median contributions for each 
candidate for each month.
...
ggplot(aes(x=contb_receipt_amt),data=ContributionsNO)+
...
### Description Two
The plot above is a density plot that bassically shows the distribution of
contributions for eachCandidate(excluding the outliers we removed earlier) .

So for instance, we can see Jeb Bush's contributions are completely
differentiated from the other candidates in that they are centered the furthest
to the left(around 2500), and are highly focused around that range. So if we 
remember from before that Jeb Bush doesn't score high in terms of the number
of contributions he's received. The size of those donations are quite large,
and in general his fund raising follows a completley differnet pattern than
those of his peers.

Note: Because the density plot
in this instance is on the log-transformed data, the density values are
much larger since the range of values has been highly compressed.The axis
shows the contribution values re-transformed to their original values, but 
we actually use the transformed values in our calculations.

### Plot Three
``````{r echo=FALSE, message=FALSE, warning=FALSE, fig.width = 20,fig.height =10, Plot_Three}
#   Histogram of Contribution bucket frequency by candidate
ggplot(aes(x=contb_receipt_amt.Bucket),data=ContributionsNO)+
         geom_histogram()+
  xlab('Contribution Amount[$]')+
  ylab('Frequency')+
...
# Description Three
The plot is a histogram faceted by candidate. With contribution amounts broken 
down by bucket on the x-axis. The 'buckets' in this instance, are
the same ones we defined earlier in the Analysis, and are being used
for the same reasons.

We can see that Ben carson has the most normalized distribution of Contributions
made out to him using log10 and buckets on the x axis. Jeb Bush's distribution 
is centered further to the right at the $500-$1000 range, with a number of
larger and smaller donations above and below this. The total frequency of his contributions seems to be lower. Bernie sanders seeems to actually mimic
ben Carson a lot in terms of the frequency and scale of his distributions, 
however, it's easy to see why Hillary has been so successful fund raising,
as she is represented strongly across all of the buckets.
